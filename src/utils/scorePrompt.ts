...
export function scorePrompt(input: string, language: Language): QualityScore {
  ...
  const actionVerbsAr =
    /اكتب|أنشئ|اشرح|حلّل|حلل|صمّم|صمم|أنتج|قارن|ابحث|طوّر|طور|أعدّ|أعد|ضع|ارسم|احسب|رتّب|رتب|لخّص|لخص|ترجم|حوّل|حول|أصلح|حسّن|حسن|اقترح|حدّد|حدد|استخرج|اكتشف|نفّذ|نفذ|خطّط|خطط|راجع|قيّم|قيم|وثّق|وثق|اختبر|نشر|اضبط|تحقّق|تحقق|اذكر|اعرض/;

  if (isFa ? actionVerbsFa.test(text) : isAr ? actionVerbsAr.test(text) : actionVerbsEn.test(lower)) clarity += 5;
  ...
  const vagueAr = /شيء ما|أي شيء|بطريقة ما|نوع من|شيء جيد/;
  if (!(isFa ? vagueFa.test(text) : isAr ? vagueAr.test(text) : vagueEn.test(lower))) clarity += 3;
  ...
  // Use [؀-ۿ] instead of \w — \w only matches ASCII in JS
  const contextPatternsFa = [
    /برای\s+[؀-ۿ]/,
    /به عنوان\s+[؀-ۿ]/,
    /من\s+(یک|دارم|هستم|کار|می‌خواهم|می‌خوام)/,
    /(شرکت|تیم|پروژه|کسب‌وکار|اپ|وب‌سایت|مشتری|کاربر|مخاطب|استارتاپ|برند)\s*(ما|من)/,
    /زمینه|پس‌زمینه|وضعیت|فعلاً|بر اساس|با فرض اینکه/,
    /مخاطب هدف|کاربر هدف/,
  ];
  const contextPatternsAr = [
    /لـ?[؀-ۿ]/,     // for [Arabic word]: لصفحة, لبيع, لتطبيق, etc.
    /في\s+[؀-ۿ]/,   // in [Arabic place/thing]
    /من أجل/,
    /أنا\s+(أحتاج|أريد|لديّ|أعمل|أملك)/,
    /نحن\s+(نحتاج|نريد|لدينا)/,
    /سياق|خلفية|بناءً على|حالياً/,
    /الجمهور المستهدف|المستخدم المستهدف|الفئة المستهدفة/,
  ];

  const pats = isFa ? contextPatternsFa : isAr ? contextPatternsAr : contextPatternsEn;
  ...
  const deliverablesAr =
    /\d+\s*(كلمة|كلمات|صفحة|جملة|خيار|مثال|بند|قسم|نقطة|خطوة|سؤال|فقرة)|كامل|شامل|موجز|احترافي|واضح|محدد|دقيق|مختصر/;
  if (isFa ? deliverablesFa.test(text) : isAr ? deliverablesAr.test(text) : deliverablesEn.test(lower)) specificGoal += 5;
  ...
  const thatClauseAr = /الذي\s+(يمكن|يساعد|يتيح|يجعل|يشرح)|لـ?(بيع|خدمة|مساعدة|تحسين|عرض)/;
  if (isFa ? thatClauseFa.test(text) : isAr ? thatClauseAr.test(text) : thatClauseEn.test(lower)) specificGoal += 4;
  ...
  const formatKeywordsAr =
    /قائمة|جدول|كود|برمجة|مقال|فقرات|بريد|تقرير|ملخص|نموذج|هيكل|قسم|خطوات|مرقّم|مرقم|منشور|تنسيق|مخطط|عرض|قاعدة بيانات|JSON|HTML|Markdown|سكريبت|وصف/;
  if (isFa ? formatKeywordsFa.test(text) : isAr ? formatKeywordsAr.test(text) : formatKeywordsEn.test(lower)) outputFormat += 8;
  ...
  const lengthSpecAr =
    /\d+\s*(كلمة|كلمات|حرف|أحرف|جملة|فقرة)|قصير|موجز|شامل|كامل|طويل|مختصر|احترافي/;
  if (isFa ? lengthSpecFa.test(text) : isAr ? lengthSpecAr.test(text) : lengthSpecEn.test(lower)) outputFormat += 4;
  ...
  const structureAr = /يشمل|يحتوي|يتضمن|مع\s+[؀-ۿ]|مقسّم|مقسم|منظّم|منظم|أقسام|أجزاء/;
  if (isFa ? structureFa.test(text) : isAr ? structureAr.test(text) : structureEn.test(lower)) outputFormat += 3;
  ...
  const constraintAr =
    /لا\s+تكتب|لا\s+تذكر|لا\s+تستخدم|تجنّب|تجنب|بدون|أبداً|أبدا|يجب|ينبغي|فقط|الحد الأقصى|الحد الأدنى|في غضون|أقل من|أكثر من|باستثناء|ركّز|ركز|أعطِ الأولوية|تأكّد|تأكد|قيد/;

  const allConstraintMatches = isFa
    ? (text.match(new RegExp(constraintFa.source, 'g')) ?? []).length
    : isAr
    ? (text.match(new RegExp(constraintAr.source, 'g')) ?? []).length
  ...
  // Arabic and Persian scripts have no uppercase — grant the bonus unconditionally for RTL
  if (text !== text.toUpperCase() || isFa || isAr) languageQuality += 3;
  ...
  const fillerAr = /أتمنى أن تساعدني في|كنت أتساءل إذا|هل بإمكانك من فضلك|إذا كان لديك وقت|أرجو إذا أمكن/;
  if (isFa ? fillerFa.test(text) : isAr ? fillerAr.test(text) : fillerEn.test(lower)) {
  ...
  const improvements = buildImprovements(breakdown, language);
...
// ─────────────────────────────────────────────────────────────────────────────
function buildImprovements(breakdown: ScoreCriteria, language: Language): string[] {
  ...
  const consider = (
    ...
    if (value / max < threshold) {
      const tip = isFa ? tipFa : isAr ? tipAr : tipEn;
      tips.push({ gap: max - value, tip });
    }
  };
  ...
  consider(
    ...
    'حدد صيغة الإخراج: قائمة، جدول، JSON، خطوة بخطوة، مقال، كود...',
  );
  consider(
    ...
    'أضف سياقاً: "لـ [الجمهور]"، "بصفتي [الدور]"، "أحتاج هذا لـ [مشروعي/شركتي]"',
  );
  consider(
    ...
    'كن أكثر تحديداً: أضف أرقاماً، عدد الكلمات، النتائج الدقيقة، أو معيار النجاح',
  );
  consider(
    ...
    'أضف قيوداً: "تجنب X"، "حد أقصى 500 كلمة"، "لا تذكر Y"، "ركز فقط على Z"',
  );
  consider(
    ...
    'استخدم فعلاً واضحاً في البداية: اكتب، أنشئ، حلّل، اشرح، قارن، صمّم...',
  );
  consider(
    ...
    'حسّن طول الـ prompt: 15–60 كلمة هو المثالي — احذف العبارات الزائدة',
  );
  consider(
    ...
    'اكتب جملاً كاملة بهيكل واضح — تجنب كلمة واحدة أو الجمل المطولة جداً',
  );
